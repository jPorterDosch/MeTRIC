...
## TODO after merge (deferred, tracked here)

- **Tier 2 — trained artifact is not consumable by the eval scripts (KNOWN GAP,
  deferred to a follow-up PR).** `finetune_depth.py` saves a
  `{"args","model","epoch"}` checkpoint whose `model` state_dict has
  LoRA-wrapped attention keys (`...attn.qkv.base.weight`) plus DepthConditioner
  params, while every eval launcher does
  `StreamVGGT().load_state_dict(torch.load(weights), strict=True)`
  (`src/eval/mv_recon/launch.py:90`, `video_depth/launch.py`, `monodepth`,
  `pose_evaluation`). So a depth-cond checkpoint cannot be evaluated as-is.
  Follow-up: add an export helper that (a) folds each LoRA `B@A` back into its
  base projection (`W += scaling * B@A`), (b) drops the conditioner unless the
  eval model is a `MetricStreamVGGT`, and (c) writes a bare state_dict, OR teach
  the eval launchers to build `MetricStreamVGGT` + load the depth-cond
  checkpoint. Until then, evaluation of finetuned weights is not wired.
- **Isolate the branch from vendored `croco`/`dust3r` (gradual).** Step 1 done:
  `src/utils.py` holds the entrypoint helpers (`to_primitive`, `picklable_args`,
  `resolve_output_dir`, `is_rank_zero`). Next step (own PR): vendor the ~9
  `croco.utils.misc` symbols `finetune_depth.py` still imports (`MetricLogger`,
  `SmoothedValue`, `save_model`, `load_model`, `save_on_master`,
  `NativeScalerWithGradNormCount`, `get_parameter_groups`, `adjust_learning_rate`,
  `aggregate_per_view_metrics`) into `src/utils.py` — ~450-500 lines with their
  transitive distributed/grad-norm helpers. Stage 8 already exercises those
  functions end-to-end, so a verbatim copy is verifiable by re-running it.
  (Further out: `dust3r.inference`, `dust3r.datasets`, `streamvggt.loss`,
  `finetune.py` are the remaining vendored deps.)
- **`num_views` / `train.clip_len` are hashed but unread** (review finding): the
  dataset clip length is baked into the `train_dataset` string as
  `num_views=10`; the `num_views` field and `clip_len` change the experiment
  hash without changing the run. Fix: f-string-interpolate `num_views` into the
  dataset default and delete/blacklist `clip_len`.
- Remaining medium/low review findings (temporal+head-arm mask-channel
  corruption; dropped end-of-epoch wandb aggregate log for short runs; the
  unbatched-`forward` dead branch; the inference sparse-depth rank asymmetry)
  and the below-cap cleanup tier (double image-stack per forward, per-file
  GPU-blocking cache loads, the `train_one_epoch` fork of finetune.py,
  `dpt_fusion_sizes` duplicating `resize_layers` geometry).
...
## Post-review changes (maintainer review, 2026-07-07)

- **Confound rule removed** (deliberate deviation from the original spec's
  "startup assertion" deliverable, per maintainer review): experiment identity
  is now `manifest` = the flattened entrypoint config minus a blacklist of
  non-identity fields (save paths, logging cadence, workers, resume state) and
  `hash` = SHA-256 of that manifest. The output dir is
  `<save_dir>/<exp_name>_<hash10>`; the entrypoint **crashes if that directory
  exists at all** (pass `--resume <ckpt>` explicitly to continue an
  interrupted run). The manifest is written to disk and to wandb, where
  callers filter runs for comparisons (a head-vs-token pair is comparable iff
  their manifests differ only in `depth_cond.injection`).
- **Hydra → tyro** for `finetune_depth.py` only (existing trainers untouched);
  `config/finetune_depth.yaml` deleted. All knobs are dataclass fields exposed
  as CLI flags, e.g. `--depth-cond.injection head --lora.rank 16`.
- **String constants → Enums** (`EncoderType`, `InjectionType`, `TemporalType`,
  `NormType`, `HeadType`, `LoRATarget`, `SparseSimMode`) with match/case
  branching and `case _: raise ValueError` fallbacks throughout.
- **Sparse-depth simulation** is now MAE-style patch masking with
  `SparseSimMode`: `NONE` (dense), `RANDOM` (per-frame patch mask),
  `TUBE_MASK` (one mask persisted across the clip), controlled by
  `sim_patch_size` / `sim_mask_ratio`.
- **Fail-fasts added**: head-arm construction errors if a configured target
  head is `None` (no gradient signal); KV-cache path raises on S>1; injected
  token features are shape-checked; per-frame conditioning lists are
  length-checked; DPT residual count must match the fusion-scale count;
  `lora.rank > 0` unconditionally.
- **Bug fixes from audit**: grad-checkpoint lambdas no longer pass `pos`
  (possibly None with RoPE disabled) as a checkpoint input — captured via
  default args instead.
- Smoke/synthetic-clip code moved out of the entrypoint into `tests/synthetic.py`;
  stage 7 now drives both arms through the entrypoint's `build_model` directly.
- **Output heads now train in BOTH arms** (`train.train_heads`, default
  `[depth, point]`; camera/track stay frozen). This supersedes the original
  spec's frozen-heads reading: the heads must be free to move for the output
  to become metric, and the knob is part of the hashed manifest so the two
  arms remain comparable. Zero-init equivalence at init is unaffected
  (freezing changes gradients, not the forward pass).
- No `assert` statements remain in library/entrypoint code (explicit
  ValueError/RuntimeError instead); test files keep asserts idiomatically.
