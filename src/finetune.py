...
def get_vis_imgs_new(loss_details, num_imgs_vis, num_views, is_metric):
    ...
    for i in range(0, num_views, stride):
        gt_imgs = (
            0.5 * (loss_details[f"gt_img{i + 1}"] + 1)[:num_imgs_vis].detach().cpu()
        )
        ...
        if f"conf_{i + 1}" in loss_details:
            cross_view_conf = (
                loss_details[f"conf_{i + 1}"][:num_imgs_vis].detach().cpu()
            )
        ...
        img_mask_list = batch_append(
            img_mask_list,
            loss_details[f"img_mask_{i + 1}"][:num_imgs_vis]
            .detach()
            .cpu()
            .unbind(dim=0),
        )
        ray_mask_list = batch_append(
            ray_mask_list,
            loss_details[f"ray_mask_{i + 1}"][:num_imgs_vis]
            .detach()
            .cpu()
            .unbind(dim=0),
        )
